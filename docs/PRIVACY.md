...
- 你创建的**凭据与管理数据**：工作区、项目、链接地址、环境标签、用户名、密码、备注、补充字段、项目说明文档、待办。
- 应用**设置 / 偏好**（如自动锁定时长、首页看板布局、同步配置）。
...
- 所有保险箱数据在写入磁盘前先在**本地加密**：AES-256-GCM，密钥由你的**主密码**经 Argon2id / PBKDF2 派生。主密码与派生密钥**永不离开你的设备**。
- 静态数据存于本地扩展存储（`chrome.storage.local`），明文凭据仅在保险箱解锁期间短暂存在于内存。

## 4. 可选的自托管同步

- 同步**默认关闭**，需手动开启。开启后，扩展只会把**已加密的保险箱（密文）**上传到**你自己配置并运营**的同步服务器。
- 所有同步流量走 HTTPS/TLS。服务器只存密文，**运营者无法解密**你的数据，对你的凭据与主密码零知识。
- 你可随时关闭同步并删除远端副本。

## 5. 可选的第三方天气服务

- 首页的天气卡片**默认关闭**。仅当你在设置中显式开启并填写城市后，扩展才会把**城市名**发送给第三方天气服务 `open-meteo.com`，用于获取该城市天气。
- 该请求**不包含**你的任何凭据或主密码，城市名不与你的身份关联、不被我们存储。不开启该卡片就不会有任何此类请求。
...
如有疑问或数据相关请求，请联系：`contact@yzs.ai`。
